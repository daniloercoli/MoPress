...
public class BlogController {
  ...
  //le utilizzo per momorizzare nel RMS il dettaglio del blog
	...
	public BlogController(MainCanvas mainCvn) {
	...
	private String sanitizeBlogName(String blogName){
		...
		if (blogName.length() >= 15){
			toReturn =  blogName.substring(0, 15);
		}
		else toReturn = blogName;
		return toReturn;
	}
	...
	public void updateDraftPost(Post aDraft, Blog aBlog, int aDraftId)throws RecordStoreException, IOException {
	    ...
	    try {
	        ...
	        aDraft.setAuthoredOn_GMT(readNullSafeDate(data));
    ...
    try {
        ...
        for (int i = 0; recordEnum.hasNextElement(); i++) {
            ...
            if (recordId != 1) {
                ...
                draftList[++i] = new Post(aBlog, readNullSafeString(data), readNullSafeString(data), null, null, null);
                data.close();
            } else {
...
public Blog loadBlog(String aName) throws RecordStoreException, IOException {
    ...
    try {
        ...
        writeNullSafeDate(aDraft.getAuthoredOn_GMT(), data);
    ...
    public Blog getBlog(int aIndex) {
        try {
        	//Blog aBlog =blogs[0];
            return loadBlog(getBlogNames()[aIndex]);
        //	return aBlog;
        } catch (Exception e) {
            mainCanvas.displayError("Failed to load blog: " + e.getMessage());
            return null;
        }
    }
     ...
     * @return
     */
    public boolean addBlog(Blog aBlog, boolean saveOnRMS){
        String name = aBlog.getBlogName();
        Blog[] newBlogNames = new Blog[blogs.length + 1];

        for (int i = 0; i < blogs.length; i++) {
            if (blogs[i].getBlogName().equals(name)) {
            	 mainCanvas.displayErrorBlocking("Cannot add this blog: " + name + " because another blog with same name already exist!");
            	 return false;
            }
        ...
        try {
            ...
            mainCanvas.displayErrorBlocking("Failed to save blog: " + e.getMessage());
            return false;
        }
